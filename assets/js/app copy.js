...
Hooks.VoteHeatMap = {
  mounted() {
    ...
    this.heatLayer = L.heatLayer(this.formatHeatPoints(this.points), {
      radius: 25,
      blur: 20,
      maxZoom: 17
    }).addTo(this.map)
  ...
  updated() {
    ...
    if (this.heatLayer) {
      this.heatLayer.setLatLngs(this.formatHeatPoints(points))
    }
  ...
  formatHeatPoints(points) {
    return points
      .filter(p => p.lat != null && p.lng != null)
      .map(p => [parseFloat(p.lat), parseFloat(p.lng), 1.0])
  },
  ...
  fitToPoints(points) {
    ...
    if (validPoints.length === 1) {
      this.map.setView(validPoints[0], 13)
      return
    }

    this.map.fitBounds(validPoints, { padding: [30, 30] })
  }
}
